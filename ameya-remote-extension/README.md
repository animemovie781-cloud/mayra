# Ameya Remote Session (IDE Extension)

This extension acts as a high-performance bridge between the **Ameya Android App** and your development environment. It enables seamless remote interaction, allowing you to read, analyze, and modify code directly from your mobile device.

## Features

- **WebSocket Bridge**: Establishes a secure real-time connection using WebSocket (default port: `8765`).
- **Universal Workspace Access**: Provides the Ameya Android app with full access to project files, terminal, and editor actions across supported development environments.
...
## Installation 
1. From Android, Go to https://github.com/nazrielnr/ameya
2. Download and Install the APK file.
3. Open your IDE (e.g., VS Code).
4. Search for `Ameya Remote Session` in the marketplace or install the extension package manually.
5. Ensure the Ameya Android app is installed on your mobile device.

## Getting Started

1. **Start the Server**:
   - **Method A (Pillbar)**: Click on the **Ameya** status item in your IDE's Pillbar.
   - **Method B (Command Palette)**: Open the Command Palette (`Ctrl+Shift+P` or `Cmd+Shift+P`) and run `Ameya Remote: Start Server`.
   - Use the provided connection details (or scan the QR code if available) to pair with the mobile app.

2. **Connect from Mobile**:
   - Open the Ameya app on Android.
...
3. **Verify Connection**:
   - Use the `Ameya Remote: Diagnose Connection` command to ensure the bridge is active and reachable.
...
For more details on the Ameya ecosystem, see the [Root README](../README.md).

...
export class MessageLifecycleController {
    ...
    public startAutoAttach() {
        ...
        this.autoAttachInterval = setInterval(async () => {
            ...
            console.log(`[Ameya MessageHandler] Auto-attach stream for ${activeId}`);
            for (const ws of wss.clients) {
                if (ws.readyState === WebSocket.OPEN) {
                    this.deps.handleGetState(ws as WebSocket.WebSocket).catch((e: any) => {
                        console.error('[Ameya MessageHandler] Auto get_state failed:', e?.message || e);
                    });
                }
            }
        }, 2000);
    ...
    public updateServer(wss: WebSocket.Server | null) {
        ...
        console.log(`[Ameya MessageHandler] Server updated. Local IP: ${localIp}`);
        if (wss) {
            this.startHeartbeat();

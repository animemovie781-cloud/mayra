...
export class ConversationStateController {
    ...
    public async handleGetState(ws: WebSocket.WebSocket): Promise<void> {
        ...
        if (streamingCandidates.length > 0) {
            ...
            console.log(`[Ameya MessageHandler] Using streaming conversation as active: ${activeId}`);
        }
        ...
        if (!activeId) {
            ...
            if (attachedCandidates.length > 0) {
                ...
                this.deps.state.activeSessionId = activeId;
                console.log(`[Ameya MessageHandler] Using attached stream conversation as active: ${activeId}`);
            }
        }
        ...
        if (!activeId) {
            ...
            if (lastActive) {
                ...
                console.log(`[Ameya MessageHandler] Restored active conversation from last active session: ${activeId}`);
            }
        }
            ...
            try {
                ...
                if (metas && metas.length > 0) {
                    ...
                    if (latest?.id) {
                        ...
                        console.log(`[Ameya MessageHandler] Inferred active conversation: ${activeId}`);
                    }
                }
            } catch (error: any) {
                console.error('[Ameya MessageHandler] Failed to infer active conversation:', error.message);
            }
        }

        if (activeId) {
            console.log(`[Ameya MessageHandler] Broadcasting active_conversation: ${activeId}`);
        ...
        if (activeId) {
            try {
                ...
                console.error('[Ameya MessageHandler] Failed to fetch state messages:', error.message);
            }
    ...
    public async handleLoadConversation(id: string, ws: WebSocket.WebSocket): Promise<void> {
        ...
        try {
            ...
            console.error('[Ameya MessageHandler] Failed to load conversation messages:', error.message);
        }

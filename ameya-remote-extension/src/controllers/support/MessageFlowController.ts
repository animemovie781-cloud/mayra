...
export class MessageFlowController {
    ...
    public async handleSendMessage(
        ...
        console.log(`[Ameya MessageHandler] handleSendMessage: content="${content.substring(0, 50)}...", attachments=${attachments.length}`);
        if (attachments.length > 0) {
            attachments.forEach((attachment, idx) => {
                console.log(`[Ameya MessageHandler] Attachment[${idx}]: mimeType=${attachment.mimeType}, dataLen=${attachment.dataBase64?.length || 0}, fileName=${attachment.fileName || 'N/A'}`);
            });
        }
        ...
        if (!modelsCache || now - modelsCache.timestamp > 10000 || !this.deps.state.models.length) {
            try {
                ...
                console.log('[Ameya MessageHandler] Refreshed quota info before send_message');
            } catch (error: any) {
                console.error('[Ameya MessageHandler] Failed to refresh quota info:', error.message);
            }
        }
        ...
        console.log(`[Ameya MessageHandler] Send to ${targetId}, ignoreBeforeIndex: ${ignoreBeforeIndex}`);
        ...
        try {
            ...
            console.error('[Ameya MessageHandler] Send stream error:', message);
            this.deps.streamOrchestrator.setStreamingState(targetId, false, false);

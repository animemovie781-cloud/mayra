...
export class MessageHandler {
    ...
    private async handleGetEventsSince(lastSeqId: number, ws: WebSocket.WebSocket) {
        ...
        console.log(`[Ameya MessageHandler] Client requesting catch-up from seqId ${lastSeqId}. Found ${missedEvents.length} events.`);
        ...
        if (lastSeqId > maxSeqId) {
            console.log(`[Ameya MessageHandler] Client seqId ${lastSeqId} > server max ${maxSeqId}. Server likely restarted, forcing full state sync.`);

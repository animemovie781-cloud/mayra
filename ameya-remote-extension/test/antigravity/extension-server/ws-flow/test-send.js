...
ws.on('open', async () => {
    ...
    log('═══ STEP 2: Sending "hello from ameya test 123" ═══');
    received.length = 0;
    ws.send(JSON.stringify({ action: 'send_message', data: { content: 'hello from ameya test 123' } }));
    ...
    log('  - Did "hello from ameya test 123" appear in the chat input?');
